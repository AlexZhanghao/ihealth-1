...
static void OutputDebug(char* szStr)
{
...
static int memory_equal_display_error(const char* const a, const char* const b,
    ...
    for (i = 0; i < size; i++) {
        ...
        const char r = b[i];
        if (l != r) {
            cm_print_error("difference at offset %" "Id" " 0x%02x 0x%02x\n",
                           i, l, r);
            differences ++;
        }
    }
...
static int memory_not_equal_display_error(
    ...
    if (same == size) {
        cm_print_error("%""Id""bytes of %p and %p the same\n",
                       same, (void *)a, (void *)b);
        return 0;
    }
    return 1;
                                           ...
                                           struct CMUnitTestState *cm_tests)
{
    ...
    if (total_skipped) {
        print_error("[  SKIPPED ] %""Id" " test(s), listed below:\n", total_skipped);
        for (i = 0; i < total_executed; i++) {
            struct CMUnitTestState *cmtest = &cm_tests[i];
    ...
    if (total_failed) {
        print_error("[  FAILED  ] %""Id" " test(s), listed below:\n", total_failed);
        for (i = 0; i < total_executed; i++) {
            struct CMUnitTestState *cmtest = &cm_tests[i];
...
int _run_tests(const UnitTest * const tests, const size_t number_of_tests) {
    ...
    print_message("[==========] Running %""Id" " test(s).\n",
    ...
    print_message("[==========] %""Id" " test(s) run.\n", tests_executed);
    print_error("[  PASSED  ] %""Id" " test(s).\n", tests_executed - total_failed);
    ...
    if (number_of_test_states != 0) {
        print_error("[  ERROR   ] Mismatched number of setup %""Id" " and "
                    "teardown %""Id" " functions\n", setups, teardowns);
